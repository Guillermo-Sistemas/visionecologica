...
$ajuste=0; $estado="Pendiente de Pago"; $motivo="";  $remision="";
...
if($abono==$totalfactura){
    ...
    $nombrecliente= $query_cliente1["nombrec"];
     ...
     $pdf->Cell(115,4,utf8_decode($ciudad),0,1,'C');
      ...
      if ($remision!=""){
        $pdf->Cell(25,4,utf8_decode('Remisión:'),0,0,'');
        $pdf->Cell(25,4,utf8_decode($remision),0,1,'');
        }else
            $pdf->Ln(4);
      ...
      //$pdf->Cell(60,4,'Metodo de pago: Tarjeta',0,1,'');
       ...
       $pdf->Cell(25, 10, 'Peso',0,0,'L');
       $pdf->Cell(30, 10, 'Valor/kilo',0,0,'L');
...
if($result > 0){
    while($data=mysqli_fetch_array($query)){
        ...
        $nompro=substr($data["nomproducto"], 0, 17);

        $pdf->Cell(40, 10, $nompro, 0);
        $pdf->Cell(25, 10, $data["cantidad"], 0);
        $pdf->Cell(30, 10, "$".$data["valorkilo"], 0);
        $pdf->Cell(20, 10, "$".$data["subtotal"], 0);
        $pdf->Ln(4);
    }
