...
$empresa2=mysqli_fetch_array($empresa);
    ...
    $queryabopres=mysqli_query($conexion, "select sum(valor) as totalprestamos FROM detalleabonoprestamo 
                    WHERE DATE(fecha)='$fecha' AND procedencia=1 ");
    $queryabopres1= mysqli_fetch_array($queryabopres);
    $totalprestamos= $queryabopres1["totalprestamos"];

    $final=$saldoanterior+$totalbase+$totalventas-($totalcompras+$totalgastos)-$saldo-$pago+$cobro+$totalprestamos;
    ...
    if($controlinicio==1)
    {
        ...
        if ($totalventas>0){ 
            ...
            if ($totalprestamos>0){ 
                ...
                $pdf->Cell(70,7, "Pago de Préstamos:", 0 , 0 , 'L', 0 );
                $pdf->SetFont('Arial','',14);
                $tp=number_format($totalprestamos, 0, ",", ".");
                $pdf->Cell(110,7, "$ ". $tp , 0 , 1 , 'L', 0 );
            }
        ...
        $totaling=$saldoanterior+$totalbase+$totalventas+$totalprestamos;
...
if ($totalgastos>0){
        $pdf->Ln(6);
        $pdf->SetFont('Arial','B',16);
        $pdf->Cell(180,4,utf8_decode('GASTOS EN EFECTIVO'),0,1,'C');
        // Salto de línea
        $pdf->Ln(3);
        ...
        $pdf->SetFont('Helvetica', 'B', 14);
        $pdf->Cell(41, 10, 'TIPO', 0);
        $pdf->Cell(54, 10, 'TERCERO',0,0,'L');
        $pdf->Cell(72, 10, 'NOTA',0,0,'C');
        $pdf->Cell(27, 10, 'VALOR',0,0,'L');
        
        $pdf->Ln(10);

            $query=mysqli_query($conexion,"SELECT g.idtipogasto, g.idcliente, g.nota, g.valorgasto, u.nombre FROM gasto g INNER JOIN
                                            usuario u ON g.idusuario=u.idusuario  WHERE idcuadre='$idcuadre' AND procedencia=1 ");
            $result =mysqli_num_rows($query);
            ...
            if($result > 0){
                while($data=mysqli_fetch_array($query)){
                    ...
                    $gasto=substr($gasto,0,15);
                    $pdf->Cell(41, 8, $gasto, 1,0,'L');
                    $idcliente=substr($idcliente,0,16);
                    $pdf->Cell(54, 8, $idcliente, 1,0,'L');
                    $pdf->Cell(72, 8, $nota, 1,0,'L');

                    $gt=number_format($data["valorgasto"], 0, ",", ".");

                    $pdf->Cell(27, 8, $gt, 1,1,'R');
