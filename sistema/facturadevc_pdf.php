...
$actual = Date('Y-m-d H:i:s', time());
    ...
    if(!empty($_GET['id'])){
        ...
        $inicio=mysqli_query($conexion, "select MAX(nodevolucionc) from devolucionc");
    ...
    $factura=mysqli_query($conexion, "select * from devolucionc WHERE nodevolucionc='$numfactura'");
    $factura2=mysqli_fetch_array($factura);

    $fecha=$factura2["fechadevc"];
    $cajero= $factura2["usuario"];
    $codcliente= $factura2["codcliente"];
    $total= $factura2["total_devc"];
    ...
    $nombrecliente = substr($nombrecliente, 0,22);
        ...
        $pdf->Cell(43,4,'DEVOLUCIÓN COMPRA:',0,0,'');
...
$pdf->SetFont('Helvetica', '', 8);
    ...
    $query=mysqli_query($conexion,"SELECT * FROM detalledevc WHERE nodevolucionc='$numfactura'");
